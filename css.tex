...
authentication information, such as HTTP authentication or session
...
Many consumer electronic devices provide a web interface along with a variety of services (e.g. FTP.) Unfortunately, a content that appears benign to one service may become malicious when mis-interpreted by another service. A cross channel scripting (XCS) attack~\cite{xcs} abuses such interactions between the web interface and other services by injecting malicous content into web content via alternate channels, bypassing common sanitizations for web exploits.  XCS also applies to large social networking sites that provide non-web cloud APIs, where a third party application supplies malicious data to the cloud service that is unexpected by the site's sanitization policy. The SiteFirewall~\cite{xcs} proposes to block XCS attacks at the payload execution stage, requiring sites to specify the permitted external resources the site can load via a site-wide policy in a cookie. This defense is ineffective against cross-origin CSS attacks because the attacker may white list the target site in its policy.
