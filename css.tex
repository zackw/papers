...
Web applications can omit the Content-Type header or send a Content-Type of
application/x-unknown-content type if they do not know the mime type of a
document. This scenario is not common but we did see it a number of times in
our experiment. Most browsers, with the notable exception of Opera, will
attempt to load stylesheets that lack a mime type in both standards mode and
quirks mode, because there is no mime type mismatch. This behavior could open
up a server to attack if it fails to set a Content-Type header on its HTML
documents. We have not yet observed any web servers in the wild that are
affected by this vulnerability, but browsers may wish to follow Opera's lead
and block such stylesheets when loaded across origins. In any case, we
recommend that web applications always set a Content-Type header.
