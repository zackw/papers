...
as \todo{[cite: xss filters]}.  This may seem unlikely, but is readily
...
Since an HTML page is allowed to load content, such as images and
style sheets, from any site regardless of origin, the web attacker can
induce the victim's browser to send HTTP GET requests to any site.
Furthermore, if the victim happens to visit the attacker's site while
still logged into the target site, then the victim's browser will send
the victim's session credentials with any request to the target site,
even if it was induced by the attacker.
