...
In fact, most modern browsers have strict modes, or standard-compliant modes, that always require the correct MIME type when loading CSS files. However, strict modes are only triggered when the web developer specifies a doctype in the referring document. Otherwise, the quirks mode is used by default for better compatibility. The strict mode option is not a mitigation to the cross-origin CSS attack because the attacker can simply avoid enabling strict mode by not specifying a doctype in their malicious documents.

\subsubsection{Conservative Approach}
To address web site compatibility concerns, we propose a conservative approach that blocks most attacks while tolerating MIME type misconfiguration. In order to reduce false positives in the strict MIME type approach, an additional level of checking is applied to cross-origin CSS loads that have invalid MIME types. When a valid MIME type is not provided, the browser will try to parse the cross-origin CSS but bailing on first syntax error. This sniffing mechanism is to guess whether the imported CSS file is actually an injected target document. Therefore, the devised solution blocks a CSS load when all the following conditions are met:
\begin{itemize}
...
(should modify this section with new experiment results)
...
\caption{Cross-Origin CSS MIME Errors}
\end{figure}

\subsubsection{Adoption}
Our proposal requires no changes to existing web servers and only modifies the browser. We implemented the conservative approach of stricter CSS loading in a patch to WebKit, the open source web browser engine component integrated in Safari and Google Chrome. The WebKit patch was first deployed in Google Chrome 4.0.249.78, and also accepted in Safari 4.0.5. Note that the cross-origin CSS raw text leak is tightened by this patch because read access to cross-origin CSS raw text is limited to well-formed imports. Inspired by our WebKit patch, the exact same heuristic was adopted in Opera 10.10. Mozilla proposes to deploy stricter MIME type checking for cross-origin CSS loads in Firefox 3.6 (version undetermined).

The implementation of accepted MIME types on each browser slightly differs as shown in the table. Most browsers accept files with an empty MIME type or application/x-unknown-content-type, which typically triggers the content-sniffing algorithm. Bogus represents incorrect MIME values that lack a slash.

\begin{table}
\centering
\caption{Valid MIME Types for Cross-Origin CSS}
...
As discussed in one of the client-side defenses, using session cookies enables web sites to controlling the response of each user's request and provide user-specific styles, e.g. adjust to mobile account or printable view. The cross-origin CSS attack could be blocked if web sites do not use cookies. The web-key authentication scheme \cite{webkey} explicitly embeds user permissions in URLs without using cookies for authentication. This approach can mitigate the attack since the attacker does not know the unguessable URL. However, implementing the web-key mechanism into existing web applications is a non-trivial task for web developers.
