...
Traffic analysis is a class of techniques to extract information from
encrypted messages based on what is still observable despite the
encryption, such as the source and destination of each message, its
length, and the time it was sent.  These techniques have been
successfully used since the 19th century, if not longer, and are a
staple of modern signals intelligence.~\cite{kahn1967codebreakers,
  greenwald2013nsa} Despite the potential for severe invasion of
privacy based \emph{only} on capture of
metadata~\cite{felten2013decl}, the modern Internet is largely
unprotected against traffic analysis. Anonymizing “overlay” networks
such as Tor and I2P conceal the source and destination of traffic, at
significant cost to both latency and transfer rate, and do not conceal
the size or timing of messages.

Both theoretical and practical studies of countermeasures for size-
and timing-based traffic analysis are thin on the ground.  Concealing
size is thought to incur prohibitive overhead~\cite{ferguson2010ceng};
concealing timing is thought to be inherently in conflict with low
latency as desired for human-interactive
protocols.~\cite{acquisti2003economics} At the same time, laboratory
studies of the power of size-based traffic analysis are easy to
find~\cite{duong2012crime, herrmann2012analyzing, zhang2010sidebuster,
  cai2012touching, overlier2006locating} but may overstate the
attack's feasibility in the field.~\cite{perry2013fpcritique}

We will present results from \emph{field} studies of size-based
traffic analysis attacks on the World Wide Web specifically,
demonstrating that the attack is hampered by sheer scale, and
susceptible to server-side countermeasures applied by each individual
website, with acceptable overhead.  We also outline a design for a
next-generation transport architecture which includes countermeasures
for traffic analysis in general (not just size-based attacks).
Building these countermeasures into the design allows them to operate
at much lower levels of overhead, in turn allowing us to enable them
for \emph{all} traffic.
